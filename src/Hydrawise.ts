...
// A set of maps that helps some tasks (such as detect controller_id, just by relay_id)
interface HWCache {
    [mapName: string]: { [key: string]: any }
}
...
export class Hydrawise {
    ...
    private __cache: HWCache = {};

    private __createCache(name: string, map: { [key: string]: any }): void {
        this.__cache[name] = {...(this.__cache[name] || {}), ...map}
    }

    private __readCachedValue(name: string, key: string): any {
        if (this.__cache[name]) return this.__cache[name][key];
        return undefined;
    }
    ...
    public getZones(controller?: HydrawiseController | number): Promise<HydrawiseZone[]> {
        ...
        let promise: Promise<HydrawiseZone[]> = new Promise((resolve, reject) => {
            ...
            that.getStatusAndSchedule(controllerID).then((data: any) => {
                ...
                const zoneToControllerMap: { [key: string]: any } = {}
